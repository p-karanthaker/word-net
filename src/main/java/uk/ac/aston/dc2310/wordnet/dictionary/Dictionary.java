...
/**
 * The Dictionary class models an electronic dictionary holding the parsed data
 * from the WordNet files (wn_s.csv, wn_g.csv, wn_hyp.csv).
 * 
 * @author KTHAKER
 *
 */
public class Dictionary implements WNControllerStringOnly {
	...
	/**
	 * A HashMap which maps a string representation of a word to a set of LexicalUnit objects which have the same
	 * name/lexical unit.
	 */
	...
	public void add(LexicalUnit word) {
		String lexicalUnit = word.getLexicalUnit().toUpperCase();	// Convert all words to UPPER CASE to be used as keys.
		
		/*
		 * Check if the word is already a key in the words map. If it is, then update the current Set of LexicalUnits
		 * in the map which correspond to that word. Else, add the word to the words map with a new Set containing only
		 * the word which was passed to be added.
		 */
		...
		/*
		 * Check if the synsets map already contains the synset of the LexicalUnit (word). If it does, then update the 
		 * current Set of LexicalUnits. Else, add the LexicalUnit into a new Set of LexicalUnits (synset) and map
		 * the synset id to the new Set.
		 */
	...
	/**
	 * Sorts a Set of LexicalUnit objects by their Part of Speech numerical value, and then
	 * by their Sense Number.
	 * 
		...
		if (hyponyms.containsKey(Integer.parseInt(synsetId))) {
			...
			while (hypernyms != null) {
				for (Integer synsetId1 : hypernyms) {
					...
					/* Set hypernyms to the set which is mapped to the synset id of the last hypernym so we can continue
					 * looping through all hypernyms.
					 */
